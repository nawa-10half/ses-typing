 ...
 *
 * 使い方:
 *   1. JSON生成:   npx tsx scripts/seed-kv.ts > /tmp/courses.json
 *   2. KVに投入:    npx wrangler kv:key put --binding KV courses --path /tmp/courses.json
 *   3. ローカル確認: npx wrangler kv:key put --binding KV courses --path /tmp/courses.json --local
 */

import type { Course } from '../src/types/game.ts'
